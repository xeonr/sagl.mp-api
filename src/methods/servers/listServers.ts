import { FieldName, ListServersRequest, ListServersRequest_ListServersRequestFilter, ListServersRequest_Sort, ListServersResponse, Operator } from '@buf/xeonr_sagl-servers.bufbuild_es/serversapi/v1/api_pb.js';
import { Server } from '../../models';
import { mapServer } from './getServer';
import type { PipelineStage } from 'mongoose';
import { getRecentDataTimestamp } from '../../util/utils';
import * as crypto from 'crypto';

const fieldMap: { [key in FieldName]: { key: string; transform?: (key: string[]) => any } } = {
	...
	stages.push(...request.filter.map(filter => {
		...
		if (filter.operator === Operator.NOT_EQUAL) {
			if (filter.value.length <= 1) {
				return { $match: { [fieldName]: { $ne: fieldValue[0] }  } };
			}
		...
		if (filter.operator === Operator.BETWEEN) {
			return { $match: { [fieldName]: { $gte: fieldValue[0], $lte: fieldValue[1] } } };
		}

		return null;
	}).filter(i => i !== null) as PipelineStage[]);

	stages.push({ $sort: { [fieldMap[request.sort!.field].key]: request.sort?.ascending ? 1 : -1 } });

	return {
		query: [
			...stages,
			{ $limit: Math.min(100, request.limit) }
		],
		stages,
		limit: request.limit,
...
function createToken(stages: PipelineStage[], offset: number, limit: number): string {
	return encrypt(Buffer.from(JSON.stringify({ stages, offset, limit })).toString('base64'), 'luke');
}


function decodeToken(token: string): { stages: PipelineStage[], offset: number; limit: number } {
	return JSON.parse(Buffer.from(decrypt(token, 'luke'), 'base64').toString());
}

export async function listServers(request: ListServersRequest): Promise<ListServersResponse> {
	...
	if (request.requestType.case === 'filter')  {
		...
		token = createToken(decoded.stages, decoded.offset + decoded.limit, decoded.limit);
